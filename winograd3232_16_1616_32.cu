...
__global__ void winograd( signed char *input,  signed short *weight,  signed char  *output){
    ...
    // __syncthreads();
    if(tx == 0 && ty == 0) {
        const int out_start1 = (bx+1) + ((by+1)*18) + ((2*tz)*324);
        const int out_start2 = (bx+1) + ((by+1)*18) + ((2*tz + 1)*324);
...
int main(){
    ...
    fp = fopen( "./layer2.0.conv1.weight", "rb" );
    ...
    for(int i=17; i<32; i++) printf("%d, ", char (res1[i]));
    printf("\n");
