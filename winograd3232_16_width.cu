...
__global__ void conv( signed char *input,  signed char *filter,  signed char *output){
    __shared__ int s_output[4][16];
    __shared__ signed char input_smem [16][16];
    const int tx = threadIdx.x, bx = blockIdx.x, by = blockIdx.y;
    const int z = tx&15, x=tx>>4;
    const int start_xy= ((bx<<1)<<4) + ((by<<1)*544) + tx;
    input_smem[x   ][z] = input[start_xy];
    input_smem[x+4 ][z] = input[start_xy+544];
    input_smem[x+8 ][z] = input[start_xy+1088];
    input_smem[x+12][z] = input[start_xy+1632];
    s_output[x][z] = 0;
    __syncthreads();
    // if(bx==0&&by==0&&tx==0){
    ...
    for(int i=0;i<4;i++){
        for(int k=0;k<9;k++){
            ...
            atomicAdd(&s_output[i][((tx&15) + ((tx    )>>4))&15], input_smem[t][z] * filter[tx+    (k<<8)]);
            atomicAdd(&s_output[i][((tx&15) + ((tx+64 )>>4))&15], input_smem[t][z] * filter[tx+64 +(k<<8)]);
            atomicAdd(&s_output[i][((tx&15) + ((tx+128)>>4))&15], input_smem[t][z] * filter[tx+128+(k<<8)]);
            atomicAdd(&s_output[i][((tx&15) + ((tx+192)>>4))&15], input_smem[t][z] * filter[tx+192+(k<<8)]);
        }
    }
    
    __syncthreads();
    if(tx < 16) {
        const int temp = (((bx<<1)+1)<<4) + (((by<<1)+1)*544);
        const int out_start1 = tx + temp;
        const int out_start2 = tx + temp + 16;
        const int out_start3 = tx + temp + 544;
        const int out_start4 = tx + temp + 560;
...
__global__ void winograd( signed char *input,  signed short *weight,  signed char *output){
    ...
    for(int i=0; i<16; i++){  //xyごとに計算する
        atomicAdd(&I[i][(z+x   )&15], BtdB[i][z]*weight[tx    +(i<<8)]);
        atomicAdd(&I[i][(z+x+4 )&15], BtdB[i][z]*weight[tx+64 +(i<<8)]);
        atomicAdd(&I[i][(z+x+8 )&15], BtdB[i][z]*weight[tx+128+(i<<8)]);
        atomicAdd(&I[i][(z+x+12)&15], BtdB[i][z]*weight[tx+192+(i<<8)]);
    }
    __syncthreads();
    if(tx < 16) {
        const int temp = (((bx<<1)+1)<<4) + (((by<<1)+1)*544);
...
int main(){
    ...
    //warm up
    for(int i=0;i<100;i++) conv<<<dim3(16, 16), 64>>>(d_char, d_filter, d_char_out);
    //Measure
    cudaEventCreate(&start);
    cudaEventCreate(&stop);
    cudaEventRecord(start, 0);
    cudaMemset(&d_char_out, 0, sizeof(signed char)*SIZE);
    conv<<<dim3(16, 16), 64>>>(d_char, d_filter, d_char_out);
    ...
    cudaMemset(&d_char_out, 0, sizeof(signed char)*(SIZE));
    winograd<<<dim3(16, 16), 64>>>(d_char, d_wino, d_char_out);
