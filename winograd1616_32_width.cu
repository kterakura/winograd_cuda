...
__global__ void conv( signed char *input,  signed char *filter,  signed char *output){
    __shared__ int s_output[4][32];
    __shared__ signed char input_smem [16][32];
    const int tx = threadIdx.x, bx = blockIdx.x, by = blockIdx.y;
    const int z = tx&31, x=tx>>5;
    const int start_xy= ((bx<<1)<<5) + ((by<<1)*576) + tx;
    input_smem[x   ][z] = input[start_xy];
    input_smem[x+4 ][z] = input[start_xy+576];
    input_smem[x+8 ][z] = input[start_xy+1152];
    input_smem[x+12][z] = input[start_xy+1728];
    s_output[x][z] = 0;
    
    // __shared__ signed char filter_smem [9216];
    // for(int i=tx;i<9216;i+=128) filter_smem[i] = filter[i];
    __syncthreads();
    // if(bx==0&&by==0&&tx==0){
    //     for(int i=0; i<256;i++) printf("%d ", input_smem[i>>4][i&15]);
    //     printf("\n");
    // }
    for(int i=0;i<4;i++){
        for(int k=0;k<9;k++){
            const int t = k+(k/3)+(i&1)+((i>>1)<<2);
            atomicAdd(&s_output[i][(z+x   )&31], input_smem[t][z]*filter[tx    +(k<<10)]);
            atomicAdd(&s_output[i][(z+x+4 )&31], input_smem[t][z]*filter[tx+128+(k<<10)]);
            atomicAdd(&s_output[i][(z+x+8 )&31], input_smem[t][z]*filter[tx+256+(k<<10)]);
            atomicAdd(&s_output[i][(z+x+12)&31], input_smem[t][z]*filter[tx+384+(k<<10)]);
            atomicAdd(&s_output[i][(z+x+16)&31], input_smem[t][z]*filter[tx+512+(k<<10)]);
            atomicAdd(&s_output[i][(z+x+20)&31], input_smem[t][z]*filter[tx+640+(k<<10)]);
            atomicAdd(&s_output[i][(z+x+24)&31], input_smem[t][z]*filter[tx+768+(k<<10)]);
            atomicAdd(&s_output[i][(z+x+28)&31], input_smem[t][z]*filter[tx+896+(k<<10)]);
        }
    }
    
    __syncthreads();
    if(tx < 32) {
        const int temp = (((bx<<1)+1)<<5) + (((by<<1)+1)*576);
        const int out_start1 = tx + temp;
        const int out_start2 = tx + temp + 32;
        const int out_start3 = tx + temp + 576;
        const int out_start4 = tx + temp + 608;
...
__global__ void winograd( signed char *input,  signed short *weight,  signed char *output){
    ...
    const int z = tx&31, x=tx>>5;
	__shared__ signed char input_smem [16][32];
	__shared__ int BtdB [16][32];
	__shared__ int I [16][32];
    ...
    for(int i=0; i<16; i++){  //xyごとに計算する
        atomicAdd(&I[i][(z+x   )&31], BtdB[i][z]*weight[tx    +(i<<10)]);
        atomicAdd(&I[i][(z+x+4 )&31], BtdB[i][z]*weight[tx+128+(i<<10)]);
        atomicAdd(&I[i][(z+x+8 )&31], BtdB[i][z]*weight[tx+256+(i<<10)]);
        atomicAdd(&I[i][(z+x+12)&31], BtdB[i][z]*weight[tx+384+(i<<10)]);
        atomicAdd(&I[i][(z+x+16)&31], BtdB[i][z]*weight[tx+512+(i<<10)]);
        atomicAdd(&I[i][(z+x+20)&31], BtdB[i][z]*weight[tx+640+(i<<10)]);
        atomicAdd(&I[i][(z+x+24)&31], BtdB[i][z]*weight[tx+768+(i<<10)]);
        atomicAdd(&I[i][(z+x+28)&31], BtdB[i][z]*weight[tx+896+(i<<10)]);
    }
